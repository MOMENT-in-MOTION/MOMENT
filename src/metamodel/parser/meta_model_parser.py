...
def parse_meta_model(
    ...
    Args:
        meta_model_dict (dict[str, str]): Dictionary containing the unstructured data of the MetaModel.
...
def _resolve_open_references(cls: MetaClass) -> None:
    """Check the associations of the given class for any OpenAssociations and tries to resolve them."""
...
def _build_attribute(attribute_values: dict) -> Attribute:
    ...
    if (
        ...
        raise ValueError(
            f"The attribute type '{attribute_values['attribute_type']}' is not a valid TypeOption or Enum name."
        )
...
def _test_enum_value(enum: Enum, value: str) -> Enum | None:
    """Test if the given value is a valid name or value for the given enum and return the corresponding Enum member if it is"""
    for member in enum:
